...
| <a name="module_essential_contacts"></a> [essential\_contacts](#module\_essential\_contacts) | ./modules/essential_contacts | n/a |
...
| <a name="input_bucket_pap"></a> [bucket\_pap](#input\_bucket\_pap) | Enable Public Access Prevention. Possible values are "enforced" or "inherited". | `string` | `"inherited"` | no |
...
| <a name="input_budget_calendar_period"></a> [budget\_calendar\_period](#input\_budget\_calendar\_period) | Specifies the calendar period for the budget. Possible values are MONTH, QUARTER, YEAR, CALENDAR\_PERIOD\_UNSPECIFIED, CUSTOM. custom\_period\_start\_date and custom\_period\_end\_date must be set if CUSTOM | `string` | `null` | no |
| <a name="input_budget_custom_period_end_date"></a> [budget\_custom\_period\_end\_date](#input\_budget\_custom\_period\_end\_date) | Specifies the end date (DD-MM-YYYY) for the calendar\_period CUSTOM | `string` | `null` | no |
| <a name="input_budget_custom_period_start_date"></a> [budget\_custom\_period\_start\_date](#input\_budget\_custom\_period\_start\_date) | Specifies the start date (DD-MM-YYYY) for the calendar\_period CUSTOM | `string` | `null` | no |
...
| <a name="input_random_project_id_length"></a> [random\_project\_id\_length](#input\_random\_project\_id\_length) | Sets the length of `random_project_id` to the provided length, and uses a `random_string` for a larger collusion domain.  Recommended for use with CI. | `number` | `null` | no |
...
| <a name="input_vpc_service_control_sleep_duration"></a> [vpc\_service\_control\_sleep\_duration](#input\_vpc\_service\_control\_sleep\_duration) | The duration to sleep in seconds before adding the project to a shared VPC after the project is added to the VPC Service Control Perimeter. VPC-SC is eventually consistent. | `string` | `"5s"` | no |
